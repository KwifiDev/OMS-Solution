...
namespace OMS.DA.Repositories.EntityRepos
{
    public class UserRepository : GenericRepository<User>, IUserRepository
    {
        ...
        {
            int rawAffected = await _dbSet.Where(u => u.UserId == user.UserId)
        ...
        {
            return await _dbSet.Where(u => u.PersonId == personId).Select(u => u.UserId).FirstOrDefaultAsync();
        }
        ...
        {
            var rowAffected = await _dbSet.Where(u => u.UserId == userId)
        ...
        {
            int rawAffected = await _dbSet.Where(u => u.UserId == userId)
        ...
        public async Task<string?> GetPasswordById(int userId)
            => await _dbSet.Where(u => u.UserId == userId)
                           .Select(u => u.Password)
                           .FirstOrDefaultAsync();
